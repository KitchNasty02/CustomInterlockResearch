...
def add_cube_interlock_in_z(mesh_left, mesh_right, beam_width_layers=2, beam_height_layers=2, beam_depth_layers=2, avoidance_dist=NOZZLE_SIZE):
    """
    ...
    # if the num is within the tolerance of a cube, then go ahead and add it
...
def add_cube_interlock_in_y(mesh_left, mesh_right, beam_width_layers=2, beam_height_layers=2, beam_depth_layers=2, avoidance_dist=NOZZLE_SIZE):
    """
    Adds a cube-based interlock between two mesh halves
    ...
    # print(f'num interlock cubes horiz: {num_int_horiz}')
    ...
    # define interlock cubes
    cube_d = beam_depth_layers*NOZZLE_SIZE
    cube_w = beam_width_layers*LAYER_HEIGHT 
    cube_h = interlock_height
    cube_size = [cube_d, cube_w, cube_h] # in mm 
    ...
    for i, w in enumerate(pattern_start_widths):
        ...
        # alternate union and diff + translate
        if i % 2 == 0:
            cube.apply_translation([plane_x + cube_d/2, w, min_int_z + cube_h/2])
             # add cube to left mesh
            mesh_left = trimesh.boolean.union([mesh_left, cube], engine='manifold')
            # subtract cube to right mesh
            mesh_right = trimesh.boolean.difference([mesh_right, cube], engine='manifold')
...
def main():
    ...
    mesh_left_with_interlock, mesh_right_with_interlock = add_cube_interlock_in_y(mesh_left, mesh_right, beam_depth_layers=4)
